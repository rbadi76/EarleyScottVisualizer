...
class EarleyScott
{
    ...
    async parseAsync4_productionStartingWithTheNonTerminal(i, productionsStartingWithTheNonTerminal, originalElement)
    {
        ...
        let deltaIsInSigmaN = cursorIsInFrontOfNonTerminal || cursorIsAtEnd; // Cursor can only be at end if this is an epsilon production
        let earleyItemAlreadyInEi = this._E[i].some(item => item.productionOrNT.isEqual(newProductionWithDotAtBeginning));
        if(deltaIsInSigmaN && !earleyItemAlreadyInEi)
        {
            this._E[i].push(newEarleyScottItem);
            this._R.push(this.cloneEarleyScottItem(newEarleyScottItem)); // Make sure to clone item.
        ...
        // Note the difference from original paper here as the array is 0-based but the string is 1-based in paper.
        let firstSymbolOfRhsEqualsToken = this._tokens[i] == production.rhs[0];
        let earleyItemAlreadyInQ = this._Q.some(item => item.isEqual(newEarleyScottItem));
        if(firstSymbolOfRhsEqualsToken && !earleyItemAlreadyInQ) 
        {
    ...
    async parseAsync6_CursorIsAtEnd_if1(i, element)
    {
        ...
        {
            let whichBranch = [];
            whichBranch.push("COMPLETER, Earley set " + i + ". if Λ=(D ::= α·,h,w) which is "+ element.toString() + ", sub-branch if w = null.")
            let D_node = new Node(element.productionOrNT.lhs, i, i, this._terminals, this._nonTerminals);
            ...
            if(arr.length === 0)
            {
                v = D_node;
                this._V.push(v);
                whichBranch.push("No node v=" + D_node.toString() + " found. Created it and added to queue V.");
            }
            else
            {
                v = arr[0];
                whichBranch.push("Node v=" + D_node.toString() + " found in queue V.");
            }
            element.w = v;
            whichBranch.push("w set to v.")
            updateParseStatus(parseStatus, this, whichBranch.join(" "));
            parseStatus.parseStatus.incrementLastStepShown();
    ...
    async parseAsync6_CursorIsAtEnd_innerIf(i, element)
    {
        ...
        if(!element.w.hasFamily(unaryFamilyWithEpsilonNode))
        {
            ...
            updateParseStatus(parseStatus, this, "In COMPLETER, Earley set "+ i + ". w = " + element.w.toString() + " in " + element.toString() + " did not have a family of epsilon. Epsilon added as a child.");
            parseStatus.parseStatus.incrementLastStepShown();
    ...
    async parseAsync6_CursorIsAtEnd_if2(i, element)
    {
        ...
        if(element.i == i)
        {
            ...
            this._H.push(h_item);
            updateParseStatus(parseStatus, this, "In COMPLETER, Earley set "+ i + ". h equals i, so we added " + h_item.toString() + " to queue H.");
            parseStatus.parseStatus.incrementLastStepShown();
            continueIfAllowed(() => this.parseAsync7_CursorIsAtEndForLoop(i, element, false));
        }
        else
        {
            updateParseStatus(parseStatus, this, "In COMPLETER, Earley set "+ i + ". h is not equal to i.");
            parseStatus.parseStatus.incrementLastStepShown();
            continueIfAllowed(() => this.parseAsync7_CursorIsAtEndForLoop(i, element, false));
        }
    }
    ...
    async parseAsync7_CursorIsAtEndForLoopItemsInE(i, element, queue, item)
    {
            ...
            && item.productionOrNT.nonTerminalAfterCursor(this._nonTerminals) == element.productionOrNT.lhs)
        {
            ...
            whichBranch.push("Item on correct format. Have called MAKE_NODE which returned y = " + y.toString());
            // production.cursorIsInFrontOfNonTerminal or cursorIsAtEnd() fulfills the criteria of delta being in ΣN.
            if((newEarleyScottItem.productionOrNT.cursorIsInFrontOfNonTerminal(this._nonTerminals) || newEarleyScottItem.productionOrNT.cursorIsAtEnd())
                && !this._E[i].some(innerItem => innerItem.productionOrNT.isEqual(newEarleyScottItem.productionOrNT) 
                && innerItem.i == newEarleyScottItem.i && innerItem.w == y))
